...
use crate::view::components::{button as btn, card};
use iced::{
...
pub fn view<'a>(
    ...
    let selector = row![
        ...
        pick_list(preset_names.clone(), selected, |name| {
            AppMessage::PluginMessage(
                0,
                PluginMsg::Edit("preset_select".to_string(), "preset".to_string(), name),
            )
        })
    ...
    let save_btn = btn::small_primary(
        text("💾 Save Current Config"),
        AppMessage::PluginMessage(0, PluginMsg::OpenModal("save_preset".to_string())),
    );
    ...
    column![
        text("Presets").size(18).font(iced::font::Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        }),
        row![selector, save_btn]
            .spacing(20)
            .align_y(iced::Alignment::Center),
